...
method getInt*[T](c: TypedCol[T], i: int): int =
  int(c.data[i])

method getGeneric*(c: Column, i: int, R: typedesc): R {.base.} =
  raise newException(AssertionError, "abstract base method called")

method getGeneric*[T](c: TypedCol[T], i: int, R: typedesc): R =
  R(c.data[i])

let c: Column = TypedCol[string](data: @["1", "2", "3"])
#echo c.getInt(0)
#echo c.getGeneric(0, int)

#[
Learnings:
- The existence of TypedCol[string] means that the compiler will create an
